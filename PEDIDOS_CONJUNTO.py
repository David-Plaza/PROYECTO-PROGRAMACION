...
# pedidos_sin_procesar = pedidos_sin_procesar()
# print(pedidos_sin_procesar)
...
def planificar_envio(pedido_id):
    ...
    print(envios_archivo)
    dump_json('envio.json', envios_archivo)

    return envio
# planificar_envio('P003')
...
def procesar_pedido(pedido_id):
    planificar_envio(pedido_id)
    actualizar_almacén_sumar_restar(pedido_id)
...
# procesar_pedido('P002')



def ver_pedidos_en_marcha():
    envios_en_marcha = cargar_json('envio.json')
    if not envios_en_marcha:
        print('No hay envíos tramitados.')
        return
    

    for n in range(len(envios_en_marcha)):

        print(envios_en_marcha[n]['Pedido_id'])
        print(f'El envío del pedido {envios_en_marcha[n]['Pedido_id']} está en marcha')
        print(f'Llegarán el día {envios_en_marcha[n]['Datos']['Fabrica']['fecha_envio']}:')
